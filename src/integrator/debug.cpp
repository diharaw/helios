...
void DebugIntegrator::execute(RenderState& render_state)
{
    ...
    vkCmdBindPipeline(render_state.cmd_buffer()->handle(), VK_PIPELINE_BIND_POINT_RAY_TRACING_KHR, m_pipeline->handle());
    ...
    vkCmdBindDescriptorSets(render_state.cmd_buffer()->handle(), VK_PIPELINE_BIND_POINT_RAY_TRACING_KHR, m_pipeline_layout->handle(), 0, 7, descriptor_sets, 0, nullptr);

    VkDeviceSize prog_size = rt_props.shaderGroupBaseAlignment;
    VkDeviceSize sbt_size  = prog_size * m_pipeline->shader_binding_table()->groups().size();

    const VkStridedBufferRegionKHR raygen_sbt   = { m_pipeline->shader_binding_table_buffer()->handle(), 0, prog_size, sbt_size };
    const VkStridedBufferRegionKHR miss_sbt     = { m_pipeline->shader_binding_table_buffer()->handle(), m_sbt->miss_group_offset(), prog_size, sbt_size };
    const VkStridedBufferRegionKHR hit_sbt      = { m_pipeline->shader_binding_table_buffer()->handle(), m_sbt->hit_group_offset(), prog_size, sbt_size };
...
void DebugIntegrator::create_pipeline()
{
    ...
    m_pipeline = vk::RayTracingPipeline::create(backend, desc);
}
