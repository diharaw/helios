...
struct Vec3i
{
    ...
    Vec3i(const Vec3i& v) :
        x(v.x), y(v.y), z(v.z) {}
    Vec3i(const Vec3f& vf) :
        x(int(vf.x)), y(int(vf.y)), z(int(vf.z)) {}
...
struct Vec4f
{
    ...
    Vec4f(const Vec3f& v, const float a) :
...
struct Vec4i
{
    ...
    Vec4i(const Vec4i& v) :
        x(v.x), y(v.y), z(v.z), w(v.w) {}
    Vec4i(const Vec3i& v, const int a) :
        x(v.x), y(v.y), z(v.z), w(a) {}
};
...
{
    return v(0, 0) * v(1, 1) * v(2, 2) - v(0, 0) * v(1, 2) * v(2, 1) + v(1, 0) * v(2, 1) * v(0, 2) - v(1, 0) * v(2, 2) * v(0, 1) + v(2, 0) * v(0, 1) * v(1, 2) - v(2, 0) * v(0, 2) * v(1, 1);
}
